...
from .face_mask.compute_dist import Face_Dist
...
# 得到face和gait融合后的dist
def dist_fusion(dist_gait, dist_face, gallery_y, probe_y):
    #dist_gait = torch.sigmoid(dist_gait)
    dist_face = torch.from_numpy(dist_face)
    dist_face = torch.sigmoid(dist_face)
    #print("===============================================dist_gait=========================================")
    #print(dist_gait)
    #print("===============================================dist_face=========================================")
    #print(dist_face)
    for (i, probe_i) in enumerate(probe_y):
        for (j, gallery_j) in enumerate(gallery_y):
            dist_gait[i][j] = dist_gait[i][j] + 20*dist_face[int(probe_i) - 75][int(gallery_j) - 75]
    return dist_gait
        ...
        for gallery_seq in gallery_seq_dict[dataset]:
            for (v1, probe_view) in enumerate(view_list):
                for (v2, gallery_view) in enumerate(view_list):
                    ...
                    dist = dist_fusion(dist_gait=dist, dist_face=dist_face, probe_y=probe_y, gallery_y=gallery_y)  # dist_face的维度是[50, 50]
